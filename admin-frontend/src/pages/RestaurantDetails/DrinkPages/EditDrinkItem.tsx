...
import { DrinkItem, Language, Category } from "../../../interfaces/Interfaces";
...
const EditDrinkItem: React.FC<EditDrinkItemProps> = ({
  drinkItem,
  onCancel,
  onSave,
  categories,
  ...
  >({
    [Language.HR]: {
      name:
        drinkItem.translations.find((t) => t.language === Language.HR)?.name ||
        "",
      description:
        drinkItem.translations.find((t) => t.language === Language.HR)
          ?.description || "",
    },
    [Language.EN]: {
      name:
        drinkItem.translations.find((t) => t.language === Language.EN)?.name ||
        "",
      description:
        drinkItem.translations.find((t) => t.language === Language.EN)
          ?.description || "",
    },
  });
  const [itemPrice, setItemPrice] = useState(drinkItem.price.toString());
  const [itemImageFile, setItemImageFile] = useState<File | null>(null);
  const [removeImage, setRemoveImage] = useState(false);
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>(
    drinkItem.categoryId || ""
  );
  const [isActive, setIsActive] = useState(drinkItem.isActive);
    ...
    <div className="py-2">
      ...
      <div className="mb-3 max-w-xl">
        ...
        <div className="mt-1 flex items-center gap-2 border border-gray-300 rounded p-2">
          ...
          >
            {drinkItem.imageUrl || itemImageFile
          ...
          {(itemImageFile || drinkItem.imageUrl) && (
            <button
