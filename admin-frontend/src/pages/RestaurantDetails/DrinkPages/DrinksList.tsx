...
interface DrinksListProps {
  ...
  onAddCategory: () => void;
  onAddDrinkItem: (categoryId?: string) => void;
  onEditCategory: (
    category: Category,
...
const DrinksList: React.FC<DrinksListProps> = memo(
  ({
    ...
    const handleEditCategory = (category: Category) => {
      // Make sure description field is explicitly included, even if empty
      const categoryWithExplicitDescription = {
        ...category,
        translations: category.translations.map((t) => ({
          ...t,
          description: t.description || "", // Ensure description is explicitly "" if empty
        })),
      };
    ...
    return (
      <div className="flex flex-col">
        ...
        <div className="border-b border-gray-200 pb-5 mb-6 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">{t("drinks")}</h2>
        ...
        <div className="space-y-8">
          {categories.map((category) => (
            <div key={category.id} className="border-b border-gray-200 pb-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                {drinkItems.filter((item) => item.categoryId === category.id)
                  .length === 0 && (
                    ...
                    >
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-1">
                            <span className="px-2 py-0.5 bg-gray-100 text-gray-800 text-sm rounded">
                              {item.price.toString().replace(".", ",")} €
                            </span>
                          </div>
                          {item.description && (
          ...
          {categories.length === 0 && drinkItems.length === 0 && (
