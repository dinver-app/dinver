...
const DrinkTab = ({ restaurantId }: { restaurantId: string | undefined }) => {
  ...
  const handleAddCategory = async () => {
    ...
    setIsLoading(true);
    try {
      const category: Category = await createDrinkCategory({
  ...
  const handleAddDrinkItem = async () => {
    ...
    try {
      ...
      const drinkItem: MenuItem = await createDrinkItem(formData);
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      const updatedCategory: Category = await updateDrinkCategory(
        editCategoryId,
        {
  ...
  const handleUpdateDrinkItem = async () => {
    ...
    try {
      ...
      const updatedDrinkItem: MenuItem = await updateDrinkItem(
        editItemId,
        formData
      );
      setDrinkItems(
        drinkItems.map((item) =>
          item.id === editItemId ? updatedDrinkItem : item
        )
      );
      setEditItemId(null);
      setEditItemName("");
  ...
  const renderDrinkItems = (categoryId: string | null) => {
    return drinkItems
  ...
  const openSortItemsModal = (categoryId: string) => {
    const itemsInCategory = drinkItems.filter(
  ...
  const handleSaveItemOrder = async () => {
    ...
    try {
      ...
      const updatedDrinkItems = [...drinkItems];
      updatedDrinkItems.sort(
  ...
  const openSortUncategorizedItemsModal = () => {
    const uncategorizedItems = drinkItems.filter((item) => !item.categoryId);
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <h3 className="section-subtitle">
            {t("manage_your_drink_items_and_categories")}
          </h3>
        </div>
            ...
            <div className="flex items-center mb-4 gap-1">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {t("add_drink_category_description")}
                </p>
              </div>
            </div>
            <div className="h-line"></div>
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative">
            ...
            <div className="flex items-center mb-4 gap-1">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {t("edit_drink_category_description")}
                </p>
              </div>
            </div>
            <div className="h-line"></div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative overflow-y-auto max-h-screen">
            ...
            <div className="flex items-center mb-4 gap-1">
              ...
              />
              <div>
                <h2 className="text-lg font-semibold">{t("add_drink_item")}</h2>
                <p className="text-sm text-gray-500">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                onClick={() => handleDeleteDrinkItem(itemToDelete.id)}
                className="delete-button"
              >
                {t("delete_item")}
              </button>
