...
interface UseRestaurantsReturn extends RestaurantsState {
  loadRestaurants: (page?: number, refresh?: boolean, search?: string, locationParams?: LocationParams) => Promise<void>;
  handleLoadMore: () => void;
  handleRefresh: () => void;
...
export function useRestaurants(
  ...
  const preloadImages = useCallback(async (restaurants: Restaurant[]) => {
    ...
    const priorityImages = restaurants.slice(0, 6)
      .map(restaurant => restaurant.iconUrl)
      .filter(Boolean);
      
    const secondaryImages = restaurants.slice(6, 15)
      .map(restaurant => restaurant.iconUrl)
      .filter(Boolean);
    ...
    try {
      ...
      if (secondaryImages.length > 0) {
        setTimeout(() => {
          Promise.all(secondaryImages.map(url => Image.prefetch(url!)))
            .catch(error => console.error('Error preloading secondary images:', error));
        }, 100);
      }
    } catch (error) {
  ...
  const loadRestaurants = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : "Unable to retrieve restaurants";
        setState(prev => ({ 
          ...prev, 
          loading: false, 
          loadingMore: false, 
  ...
  useEffect(() => {
    ...
    if (locationParams?.latitude && locationParams?.longitude && (hasLocationChanged || hasSearchChanged)) {
