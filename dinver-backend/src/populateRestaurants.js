...
const { Restaurant } = require('../models'); // Adjust the path as necessary

// Load JSON data
const dataPath = path.join(__dirname, 'restaurants.json'); // Adjust the file path as necessary
  ...
  try {
    ...
    for (const entry of limitedData) {
      await Restaurant.create({
        ...
        is_open_now: entry.opening_hours ? entry.opening_hours.open_now : null,
    ...
    console.log('Database populated with first two entries successfully!');
  } catch (error) {
